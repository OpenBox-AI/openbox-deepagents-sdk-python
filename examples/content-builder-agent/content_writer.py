...
This example demonstrates openbox-deepagent governance middleware
wrapping a deepagents application.
...
def create_content_writer():
    """Create a content writer agent with OpenBox governance middleware."""
    subagent_defs = load_subagents(EXAMPLE_DIR / "subagents.yaml")
    known_subagents = [s["name"] for s in subagent_defs] + ["general-purpose"]

    # Create OpenBox governance middleware — this is the SDK integration point
...
class AgentDisplay:
    """Formats agent streaming output with Rich terminal UI."""
...
async def main():
    ...
    console.print()
    console.print("[bold blue]Content Builder Agent[/] [dim](with OpenBox governance)[/]")
